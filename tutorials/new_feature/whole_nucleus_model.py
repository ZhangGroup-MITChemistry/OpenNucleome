...
class OpenNucleome:
    ...
    In our model, each component has specific type. Compartment A: 1, Compartment B: 2, Pericentromeres: 3, Centromeres: 4, Nucleoli: 5, dP Speckles: 6, P Speckles: 7, New Beads: 8, Lamina: 9
    '''
    ...
    def create_system(self, PDB_file, flag_membrane = False, lam_bond = None):
        ...
        PDB_file (string, required):
            The path to the chromosome PDB file, including nucleoli, speckles, new beads, and lamina.
        ...
        self.bead_groups = [[] for _ in range(self.N_type)] # group the indexes of atoms by their types; if you do not want to add some nuclear landmarks, then use the different value. compartment A: 1, compartment B: 2, regions around the centromeres: 3, centromeres: 4, nucleoli: 5, dP speckles: 6, P speckles: 7, new beads: 8, lamina: 9
        ...
        for i in range(self.N_type-1):
            if i<=self.chrbead_type-1:
                ...
                self.N_chr_nuc_spec_new += len(self.bead_groups[i])
            elif i<=self.chrbead_type:
                self.N_chr_nuc += len(self.bead_groups[i])
                self.N_chr_nuc_spec += len(self.bead_groups[i])
                self.N_chr_nuc_spec_new += len(self.bead_groups[i])
            elif i <= self.chrbead_type+1:
                self.N_chr_nuc_spec += len(self.bead_groups[i])
                self.N_chr_nuc_spec_new += len(self.bead_groups[i])
            else:
                self.N_chr_nuc_spec_new += len(self.bead_groups[i])
        ...
        self.newfeature_model = NewFeature(self.chr_system.getNumParticles(), self.N_type, self.all_bonds, self.compart_type, self.chr_groups, self.bead_groups, self.mol_type)
    ...
    def generate_element(self, flag_membrane):
        ...
        flag_membrane (bool, required):
            A flag to control the dynamics of the lamina membrane, True: include the dynamics; False: exclude the dynamics (Default: False)
        '''
        name_to_element = ['ASP', 'GLU', 'HIS', 'LYS', 'ARG', 'ASN', 'GLN', 'PRO', 'CYS']
        self.Elements = []
    ...
    def add_new_potential(self, flag):
        '''
        Add the potential related to the new beads in the system.
        ...
        '''
        if flag['new-new']:
            self.chr_system.addForce(self.newfeature_model.add_new(20))
    ...
    def add_lamina_potential(self, flag, chr_lam_param):
        ...
        '''
        if flag['lam-chrom']:
            self.chr_system.addForce(self.lamina_model.add_chr_lam(chr_lam_param, 21))
        if flag['hard-wall']:
            self.chr_system.addForce(self.lamina_model.add_hardwall(22))
        if flag['lam-lam']:
            self.chr_system.addForce(self.lamina_model.add_lam_lam(23))
        if flag['squeeze_nucleus']:
            self.chr_system.addForce(self.lamina_model.add_squeeze_nucleus(24, k = flag['strength']))
    ...
    def load_default_settings(self):
        ...
        self.add_new_potential(dict_new_ff)
